...
type booterFunc func(types.Machine) (*types.Spec, error)

func (b booterFunc) BootSpec(m types.Machine) (*types.Spec, error) { return b(m) }
func (b booterFunc) ReadBootFile(id types.ID) (io.ReadCloser, int64, error) {
	return nil, -1, errors.New("no")
}
func (b booterFunc) WriteBootFile(id types.ID, r io.Reader) error { return errors.New("no") }

func TestIpxe(t *testing.T) {
	booter := func(m types.Machine) (*types.Spec, error) {
		return &types.Spec{
			Kernel: types.ID(fmt.Sprintf("k-%s-%d", m.MAC, m.Arch)),
			Initrd: []types.ID{
				types.ID(fmt.Sprintf("i1-%s-%d", m.MAC, m.Arch)),
				types.ID(fmt.Sprintf("i2-%s-%d", m.MAC, m.Arch)),
			},
...
`
	...
	booter = func(m types.Machine) (*types.Spec, error) { return nil, nil }
	...
	// Booter error
	booter = func(m types.Machine) (*types.Spec, error) { return nil, errors.New("boom") }
	s.Booter = booterFunc(booter)
	rr = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/_/ipxe?mac=fe:fe:fe:fe:fe:fe&arch=1", nil)
	if err != nil {
...
func (b readBootFile) BootSpec(m types.Machine) (*types.Spec, error) { return nil, nil }
func (b readBootFile) ReadBootFile(id types.ID) (io.ReadCloser, int64, error) {
	d := fmt.Sprintf("%s %s", id, b)
	return ioutil.NopCloser(bytes.NewBuffer([]byte(d))), int64(len(d)), nil
}
func (b readBootFile) WriteBootFile(id types.ID, r io.Reader) error { return errors.New("no") }
