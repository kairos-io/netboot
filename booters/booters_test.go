...
func TestStaticBooter(t *testing.T) {
	...
	s := &types.Spec{
		Kernel: types.ID(filepath.Join(dir, "foo")),
		Initrd: []types.ID{
			types.ID(filepath.Join(dir, "bar")),
			types.ID(filepath.Join(dir, "baz")),
		},
	...
	expected := &types.Spec{
		...
		Initrd:  []types.ID{"initrd-0", "initrd-1"},
...
func TestAPIBooter(t *testing.T) {
  ...
  "message": "Hello from test world!"
	...
	quuxID := types.ID(spec.Cmdline[12 : len(spec.Cmdline)-16])
	fs := map[types.ID]string{
